...
import com.android.crazy.common.network.exception.ApiException
import com.android.crazy.common.network.result.Result
...
sealed class NetworkResponse<out T : Any> {
     ...
     * 业务错误
     */
    data class BizError(val code: Int = 0, val msg: String = "") :
...
fun <T : Any> NetworkResponse<T>.exceptionOrNull(): Throwable? =
    when (this) {
        ...
        is NetworkResponse.BizError -> ApiException(code, msg)
...
fun <T : Any> NetworkResponse<T>.getOrThrow(): T =
    when (this) {
        is NetworkResponse.Success -> data
        is NetworkResponse.BizError -> throw ApiException(code, msg)
